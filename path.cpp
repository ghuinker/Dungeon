...
static int32_t tunnel_cmp(const void *key, const void *with) {
  return ((int32_t) dungeon->pcunnel[((path_t *) key)->pos[dim_y]]
                                      [((path_t *) key)->pos[dim_x]] -
          (int32_t) dungeon->pcunnel[((path_t *) with)->pos[dim_y]]
                                      [((path_t *) with)->pos[dim_x]]);
}
...
void dijkstra_tunnel(dungeon *d)
{
  ...
  d->pcunnel[d->pc.position[dim_y]][d->pc.position[dim_x]] = 0;
  ...
  while ((c = heap_remove_min(&h))) {
    ...
    c->hn = NULL;
    if ((p[c->pos[dim_y] - 1][c->pos[dim_x] - 1].hn) &&
        (d->pcunnel[c->pos[dim_y] - 1][c->pos[dim_x] - 1] >
         d->pcunnel[c->pos[dim_y]][c->pos[dim_x]] +
    ...
    if ((p[c->pos[dim_y] - 1][c->pos[dim_x]    ].hn) &&
        (d->pcunnel[c->pos[dim_y] - 1][c->pos[dim_x]    ] >
         d->pcunnel[c->pos[dim_y]][c->pos[dim_x]] +
         tunnel_movement_cost(c->pos[dim_x], c->pos[dim_y]))) {
      d->pcunnel[c->pos[dim_y] - 1][c->pos[dim_x]    ] =
        (d->pcunnel[c->pos[dim_y]][c->pos[dim_x]] +
    ...
    if ((p[c->pos[dim_y]    ][c->pos[dim_x] - 1].hn) &&
        (d->pcunnel[c->pos[dim_y]    ][c->pos[dim_x] - 1] >
         d->pcunnel[c->pos[dim_y]][c->pos[dim_x]] +
         tunnel_movement_cost(c->pos[dim_x], c->pos[dim_y]))) {
      d->pcunnel[c->pos[dim_y]    ][c->pos[dim_x] - 1] =
        (d->pcunnel[c->pos[dim_y]][c->pos[dim_x]] +
